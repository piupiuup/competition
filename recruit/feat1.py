...
data = data.merge(date_info[['visit_date','holiday_flg']], on=['visit_date'],how='left')
######################################常用方法#########################################
# 合并节约内存
def concat(L):
    result = None
...
def get_label(end_date,n_day):
    ...
    if os.path.exists(result_path) & load:
        ...
        for i in [3,2,1,-1]:
            ...
            date_info_temp.rename(columns={'holiday_flg':'ahead_holiday{}'.format(i)},inplace=True)
...
def get_area_genre_week_feat(label, key, n_day):
    ...
    if os.path.exists(result_path) & load:
        ...
        start_date = date_add_days(key[0], -n_day)
        data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
        result = data_temp.groupby(['air_area_name0','air_genre_name', 'dow'], as_index=False)['visitors'].agg({'area_genre_dow_min{}'.format(n_day): 'min',
                                                                                             'area_genre_dow_mean{}'.format(n_day): 'mean',
                                                                                             'area_genre_dow_median{}'.format(n_day): 'median',
                                                                                             'area_genre_dow_max{}'.format(n_day): 'max',
                                                                                             'area_genre_dow_count{}'.format(n_day): 'count',
                                                                                             'area_genre_dow_std{}'.format(n_day): 'std',
                                                                                             'area_genre_dow_skew{}'.format(n_day): 'skew'})
...
def get_reserve_feat(label,key):
    ...
    if os.path.exists(result_path) & load:
        ...
        air_reserve_temp = air_reserve[(air_reserve.visit_date >= key[0]) & (air_reserve.visit_date < label_end_date) & (air_reserve.reserve_date < key[0])].copy()
        ...
        air_result = left_merge(label, air_result, on=['store_id','visit_date']).fillna(-1)
        hpg_result = left_merge(label, hpg_result, on=['store_id', 'visit_date']).fillna(-1)
        air_date_result = left_merge(label, air_date_result, on=['visit_date']).fillna(-1)
        hpg_date_result = left_merge(label, hpg_date_result, on=['visit_date']).fillna(-1)
        result = pd.concat([air_result,hpg_result,air_date_result,hpg_date_result],axis=1)
        result.to_hdf(result_path, 'w', complib='blosc', complevel=5)
    return result
