...
cache_path = 'F:/corporacion_cache2/'
...
# train.loc[(train.onpromotion == True), 'unit_sales'] *= 0.66 # reduce unit sales by 1/Kpromo
...
train = pd.read_hdf(data_path + 'train_20162.hdf', 'w')
label_train = pd.read_hdf(data_path + 'train_2016.hdf', 'w')
...
def get_label(end_date):
    ...
    if os.path.exists(result_path) & load:
        ...
        label = label_train[(label_train['date'] < label_end_date) & (label_train['date'] >= end_date)]
        ...
        index = label_train[(label_train['date'] < end_date) & (label_train['date'] >= '2016-06-01')]
