...
# 分组归一化
def grp_normalize(data,key,name,start=0):
    data_temp = data.copy()
    max_min = data_temp.groupby(key,as_index=False)[name].agg({'max':'max',
                                            'min':'min'})
    data_temp = data_temp.merge(max_min,on=key,how='left')
    data_temp[name] = (data_temp[name]-data_temp['min'])/(data_temp['max']-data_temp['min'])
    data_temp[name] = data_temp[name].replace(-np.inf, start)
...
def get_base_feat(data):
    ...
    feat['hy_count'] = feat['hy'].map(entbase['hy'].value_counts())
    feat['etype_count'] = feat['etype'].map(entbase['etype'].value_counts())
    ...
    feat['zczb3'] = grp_normalize(feat, 'etype', 'zczb')
    feat['mpnum3'] = grp_normalize(feat, 'etype', 'mpnum')
    feat['finzb3'] = grp_normalize(feat, 'etype', 'finzb')
    feat['mpnum3'] = grp_normalize(feat, 'etype', 'mpnum')
    feat['inum3'] = grp_normalize(feat, 'etype', 'inum')
    feat['fstinum3'] = grp_normalize(feat, 'etype', 'fstinum')
    feat['tzinum3'] = grp_normalize(feat, 'etype', 'tzinum')
    feat['ienum3'] = grp_normalize(feat, 'etype', 'ienum')
    # feat['zczb4'] = grp_normalize(feat, 'hy', 'zczb')
    # feat['mpnum4'] = grp_normalize(feat, 'hy', 'mpnum')
    # feat['finzb4'] = grp_normalize(feat, 'hy', 'finzb')
    # feat['mpnum4'] = grp_normalize(feat, 'hy', 'mpnum')
    # feat['inum4'] = grp_normalize(feat, 'hy', 'inum')
    # feat['fstinum4'] = grp_normalize(feat, 'hy', 'fstinum')
    # feat['tzinum4'] = grp_normalize(feat, 'hy', 'tzinum')
    # feat['ienum4'] = grp_normalize(feat, 'hy', 'ienum')
    hy_dummies = pd.get_dummies(feat['hy'], prefix='hy')
    feat = pd.concat([feat, hy_dummies], axis=1)
...
def get_qualification_feat(data):
...
def make_set(path):
    ...
    data_path = path
    t0 = time.time()
    result_path = cache_path + 'feat_set.hdf'
    if os.path.exists(result_path) & 0:
        result = pd.read_hdf(result_path, 'w')
    else:
        entbase = pd.read_csv(data_path + '1entbase.csv').fillna(0)
        data = entbase[['id']].copy()
        print('开始构造特征...')
        base_feat = get_base_feat(data)                # 添加基础特征
        alter_feat = get_alter_feat(data)              # alter特征
        branch_feat = get_branch_feat(data)            # branch特征
        invest_feat = get_invest_feat(data)            # invest特征
        right_feat = get_right_feat(data)              # right特征
        project_feat = get_project_feat(data)          # project特征
        lawsuit_feat = get_lawsuit_feat(data)          # lawsuit特征
        breakfaith_feat = get_breakfaith_feat(data)    # breakfaith特征
        recruit_feat = get_recruit_feat(data)          # recruit特征
