...
if os.path.exists(all_data_path):
    ...
    user_action = pd.read_csv(data_path + 'jdata_user_action.csv').fillna(0).rename(columns={'a_date':'time'})
...
def get_user_order_time_feat(data, data_key):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        for i in [30,101]:                               # 各个品类
            data_temp['user_order_cate{}_last0_time'.format(i)] = get_last_values(data_temp, user_order_temp[user_order_temp['cate'] == i], 'user_id', 'time', 'diff_of_days', shift=0)
...
def get_user_order_feat2(data, data_key, days):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        end_date = date_add_days(data_key, -days)
        stat = user_order[(user_order['time'] < data_key) & (user_order['time'] >= end_date)].copy()
        sku_ids = [11206, 16197, 31517, 75756, 55837, 51600, 31841, 84163, 93317,
            90025, 64960, 38949, 90936, 49365, 33833, 90894,  5325, 58700,
            35065, 19027]
...
# 构造候选集和label
def pre_treatment2(data_key,days):
    result_path = cache_path + 'data2_{}_{}days.hdf'.format(data_key,days)
    if os.path.exists(result_path) & 1:
        data = pd.read_hdf(result_path, 'w')
    else:
        ...
        data['end_date'] = data_key
        label = user_order[(user_order['time'] >= data_key) & (user_order['time'] < date_add_days(data_key,days))].copy()
...
def get_user_rate_feat2(data, data_key, days):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        stat2 = pd.DataFrame()
        for i in range(100):
            d = 30 if days > 30 else days
            date = date_add_days(data_key,-d-7*i)
            if date<'2016-10-01':
                break
            stat2 = stat2.append(pre_treatment2(date,d))
...
# 构造候选集和label
def pre_treatment3(data_key,days):
    result_path = cache_path + 'data3_{}_{}days.hdf'.format(data_key,days)
    if os.path.exists(result_path) & 1:
        data = pd.read_hdf(result_path, 'w')
    else:
...
def get_user_rate_feat3(data, data_key, days):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        for i in range(100):
            ...
            if date<'2016-10-01':
                break
            stat2 = stat2.append(pre_treatment3(date,d))
        stat['sku_rate'] = stat['sku_id'].map(stat2.groupby('sku_id')['label'].mean())
...
def get_user_base_feat(data,data_key):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        for i in range(1, 19):
            data_temp = data_temp.merge(pd.read_hdf(data_path + "%s.hdf" % i, 'w'), 'left', 'user_id')
        feat = data_temp
        feat.to_hdf(result_path, 'w', complib='blosc', complevel=5)
    return feat
