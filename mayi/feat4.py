...
def get_most_wifi(wifi_infos):
    if wifi_infos != '':
        bssid = sorted([wifi.split('|') for wifi in wifi_infos.split(';')],key=lambda x:int(x[1]),reverse=True)[0][0]
        return bssid
    return np.nan
...
def get_shop_wifi_tfidf(data,data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        shop_wifi = pd.DataFrame(shop_wifi, columns=['row_id', 'shop_id', 'bssid', 'Flag'])
        shop_wifi = group_rank(shop_wifi, 'row_id', 'Flag', ascending=True)
...
def clear(data,candidate,data_key):
    ...
    if os.path.exists(data_path) & os.path.exists(candidate_path) & flag:
        ...
        wifi_count_signal_dict = defaultdict(lambda: 0)
        for wifi_infos in data['wifi_infos'].values:
            for wifi_info in wifi_infos.split(';'):
                bssid, signal, Flag = wifi_info.split('|')
                wifi_count_signal_dict[bssid] += 1
        def f_1(wifi_infos):
            return ';'.join(
                [wifi_info for wifi_info in wifi_infos.split(';') if wifi_count_signal_dict[wifi_info.split('|')[0]] > 1])
        def f_2(wifi_infos):
            return ';'.join(
                [wifi_info for wifi_info in wifi_infos.split(';') if wifi_count_signal_dict[wifi_info.split('|')[0]] > 0])
        data['wifi_infos'] = data['wifi_infos'].apply(f_1)
        candidate['wifi_infos'] = candidate['wifi_infos'].apply(f_2)
...
def get_tfidf_sample(data,candidate,data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        for row in candidate.itertuples():
            ...
            if wifi_infos != '':
                wifi_infos = sorted([wifi.split('|') for wifi in wifi_infos.split(';')],key=lambda x:int(x[1]),reverse=True)
        ...
        def knn2_loss(shop_id, wifis):
            ...
            for dt_wifis in shop_wifi_signal_dict[shop_id]:
                ...
                for bssid in wifis:
                    ...
                    single_loss += (a - b) ** 2
                loss.append((2 * 10 ** 49) ** (0 - single_loss))
            if len(loss)!=0:
...
def get_multi_pred(sample):
    multi_pred = pd.read_hdf(cache_path + 'multi_pred_25-31.hdf', 'w')
...
def get_sample(data, candidate, data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = pd.concat([cwifi_sample,
                            ...
                            knn_sample,
                            people_sample,
                            loc_sample]).drop_duplicates()
        # 剔除错误样本
        shop = pd.read_csv(shop_path)
        shop_mall_dict = dict(zip(shop['shop_id'].values,shop['mall_id']))
        result = result.merge(candidate[['row_id','user_id','mall_id','longitude','latitude','time_stamp','wifi_infos']],on='row_id',how='left')
