...
for i in np.arange(0.7,1.4,0.1):
    ...
    def get_shop_wifi_mean_signal_dict(data):
        shop_wifi = []
        for row_id, ship_id, wifi_infos in zip(data['row_id'].values, data['shop_id'].values, data['wifi_infos'].values):
            for wifi_info in wifi_infos.split(';'):
                try:
                    bssid, signal, flag = wifi_info.split('|')
        ...
        shop_wifi['signal'] = shop_wifi['signal']**i
        print(i)
        shop_wifi_mean_signal_dict = (shop_wifi.groupby(['shop_id','bssid'])['signal'].mean()**(1/i)).to_dict()
    ...
    def knn_pred(row):
        ...
        result = sorted(shop_knn_loss,key=lambda x:shop_knn_loss[x])[0]
        return result
    print('开始预测...')
