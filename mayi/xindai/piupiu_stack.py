...
# 用户购买次数
def get_order_count(start_date, n_day):
    result_path = cache_path + 'order_count_{0}_{1}.hdf'.format(start_date, n_day)
    if os.path.exists(result_path) & load:
        result = pd.read_hdf(result_path, 'w')
    else:
        data_start_date = date_add_days(start_date, (0 - n_day))
        order_temp = order[(order['buy_time'] >= data_start_date) & (order['buy_time'] < start_date)]
        order_temp = order_temp.groupby('uid',as_index=False)['buy_time'].agg({'n_order_day_{}'.format(n_day):'nunique'})
        result = user[['uid']].merge(order_temp,on='uid',how='left')
        result.to_hdf(result_path, 'w', complib='blosc', complevel=5)
    return result
...
def make_feats(start_date,n_days):
    ...
    if os.path.exists(result_path) & 0:
        ...
        click_history_120 = get_click_history('2016-11-01', 120)    # 30天前是否有浏览记录
        ...
        # order_count60 = get_order_count(start_date, 60)            # 用户购买次数
        # bt_loan_history60 = get_bt_loan_history(start_date, 60)      # 获取白条历史信息
...
submission.to_csv(r'C:\Users\csw\Desktop\python\JD\xindai\submission\sub20171216_piupiu_final_stack.csv',
