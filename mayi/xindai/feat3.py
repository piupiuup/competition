...
def date_add_days(start_date, days):
    end_date = datetime.strptime(start_date[:10], '%Y-%m-%d') + timedelta(days=days)
...
def make_set(start_date):
    diff_days = diff_of_days('2016-08-03',start_date)
    data = pd.DataFrame()
    for i in range(diff_days//30):
...
def make_feats(start_date,n_days):
    ...
    if os.path.exists(result_path) & 0:
        ...
        print('开始合并特征...')
        result = concat([user_feat,loan_pred,loan_history_120,loan_history_60,loan_history_30,loan_history_7,
                         loan_history_1,loan_now_3,loan_window30,loan_last,
