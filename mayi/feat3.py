...
# knn3
def get_knn3_feat(data,sample,data_key):
    result_path = cache_path + 'knn3_feat_{}.hdf'.format(data_key)
    if os.path.exists(result_path) & flag:
        result = pd.read_hdf(result_path, 'w')
    else:
        shop_wifi_signal_dict = defaultdict(lambda: [])
        for shop_id, wifi_infos in zip(data['shop_id'].values, data['wifi_infos'].values):
            if wifi_infos != '':
                ...
                for wifi_info in wifi_infos.split(';'):
                    ...
                    wifi_signal_dict[bssid] = float(signal)
                shop_wifi_signal_dict[shop_id].append(wifi_signal_dict)
        def knn3_loss(shop_id, wifis):
            loss = []
            for dt_wifis in shop_wifi_signal_dict[shop_id]:
                single_loss = 0
                for bssid in wifis:
                    diff = wifis[bssid] - dt_wifis[bssid]
                    single_loss += abs(diff) ** 1.0
                loss.append(0.87 ** (single_loss * 1000))
            if len(loss)!=0:
                return [sum(loss)]
            return [0]
        ...
        for row in tqdm(sample.itertuples()):
            ...
            if wifi_infos != '':
                for wifi_infos in row.wifi_infos.split(';'):
                    bssid, signal, Flag = wifi_infos.split('|')
                    wifis[bssid] = float(signal)
            result.append(knn3_loss(row.shop_id, wifis))
        result = pd.DataFrame(result,columns=['knn3'])
        result.to_hdf(result_path, 'w', complib='blosc', complevel=5)
    return result

# knn4
def get_knn4_feat(data,sample,data_key):
    result_path = cache_path + 'knn4_feat_{}.hdf'.format(data_key)
    if os.path.exists(result_path) & flag:
        result = pd.read_hdf(result_path, 'w')
    else:
        shop_wifi_signal_dict = defaultdict(lambda: [])
        for shop_id, wifi_infos in zip(data['shop_id'].values, data['wifi_infos'].values):
            if wifi_infos != '':
                ...
                for wifi_info in wifi_infos.split(';'):
                    ...
                    wifi_signal_dict[bssid] = float(signal)
                shop_wifi_signal_dict[shop_id].append(wifi_signal_dict)
        def knn4_loss(shop_id, wifis):
            loss = []
            for dt_wifis in shop_wifi_signal_dict[shop_id]:
                single_loss = 0
                for bssid in wifis:
                    diff = wifis[bssid] - dt_wifis[bssid]
                    single_loss += abs(diff) ** 3.0
                loss.append(0.87 ** (single_loss * 1000))
            if len(loss)!=0:
                return [sum(loss)]
            return [0]
        ...
        for row in tqdm(sample.itertuples()):
            ...
            if wifi_infos != '':
                for wifi_infos in row.wifi_infos.split(';'):
                    bssid, signal, Flag = wifi_infos.split('|')
                    wifis[bssid] = float(signal)
            result.append(knn4_loss(row.shop_id, wifis))
        result = pd.DataFrame(result,columns=['knn4'])
        result.to_hdf(result_path, 'w', complib='blosc', complevel=5)
    return result
...
def make_feats(data, candidate):
    ...
    if os.path.exists(result_path) & 0:
        ...
        knn3_feat = get_knn3_feat(data, sample, data_key)               # knn3
        knn4_feat = get_knn4_feat(data, sample, data_key)               # knn4
        # 从单个wifi角度来判断
        # 用auc从相对大小来判断
        ...
        result = concat([sample,cwifi_feat,tfidf_feat,mwifi_feat,knn_feat,cos_feat,
                         ...
                         loc_knn_feat,multi_pred,knn3_feat,knn4_feat])
