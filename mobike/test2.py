...
def get_user_count(data, sample, data_key):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        data_temp['lat2'] = data_temp['geohashed_end_loc'].apply(lambda x: 0 if x not in loc_dict else loc_dict[x][0])
        data_temp['lon2'] = data_temp['geohashed_end_loc'].apply(lambda x: 0 if x not in loc_dict else loc_dict[x][1])
...
if __name__ == '__main__':
    ...
    orderid_sample = make_sample(data.orderid.tolist(),n_sub=20,seed=66)[0]
    del train, test

    # 生成每天的数据
    data_list = pd.date_range('2017-05-25 00:00:00', '2017-05-27 00:00:00')
    ...
    for i in range(len(data_list) - 1):
        ...
        print('#####################下一个###################')
        pool.apply_async(make_train_set, (data_temp.copy(), train_data.copy()))
    pool.close()
    pool.join()
#
# # 预测
...
# test_pred.to_csv(cache_path+'test_pred.csv', 'w', complib='blosc', complevel=5)
# test_pred = get_label(test_pred)
# bike_eloc = get_bike_next_loc(data, test, 1)
...
# result.to_csv('/home1/csw/submission/091311.csv', index=False, header=False)
# result = get_noise(result, 0.1)
# result.to_csv('/home1/csw/submission/09131.csv', index=False, header=False)
#
#
