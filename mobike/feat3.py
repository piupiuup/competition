...
def get_user_end_loc(data,candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result['user_eloc_count'] = result['user_eloc_count'] - (result['geohashed_end_loc']==result['orderid'].map(order_eloc_dict))
...
def get_loc_near_loc(data,candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        order_eloc_dict = dict(zip(data['orderid'].values, data['geohashed_end_loc'].values))
        result['eloc_n_input'] = result['eloc_n_input'] - (result['geohashed_end_loc'] == result['orderid'].map(order_eloc_dict))
...
def get_user_forward_loc(data,candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = pd.merge(data_temp[['orderid','userid','rank','starttime']],
                          data_temp_temp[['userid','rank','geohashed_start_loc','starttime']],on=['userid','rank'],how='inner')
        result['user_eloc_forward_sep_time'] = result.apply(lambda x: diff_of_minutes(x['starttime_y'],x['starttime_x']),axis=1)
        result.rename(columns={'geohashed_start_loc':'geohashed_end_loc'},inplace=True)
...
def get_user_count(data,sample):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        feat['sum_lat'] = feat['sum_lat'] - (feat['geohashed_end_loc'] == feat['orderid'].map(order_eloc_dict))\
                                            * feat['geohashed_end_loc'].apply(lambda x: np.nan if x is np.nan else loc_dict[x][0])
        feat['sum_lon'] = feat['sum_lon'] - (feat['geohashed_end_loc'] == feat['orderid'].map(order_eloc_dict)) \
                                            * feat['geohashed_end_loc'].apply(lambda x: np.nan if x is np.nan else loc_dict[x][1])
        ...
        feat['center_dis'] = ((feat['lat2'] - feat['mean_lat']) ** 2 + (np.cos(feat['mean_lon']/ 57.2958)*(feat['lon2'] - feat['mean_lon'])) ** 2) ** 0.5
...
def get_user_sloc(data,sample):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        order_sloc_dict = dict(zip(data['orderid'].values, data['geohashed_start_loc'].values))
        feat['user_sloc_count'] = feat['user_sloc_count'] - (feat['geohashed_start_loc'] == feat['orderid'].map(order_sloc_dict))
...
def get_user_eloc(data,sample,candidate):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        feat[['user_eloc_next_loc_dis','user_eloc_next_loc_mdis']] = get_distance(feat,sloc='geohashed_end_loc',eloc='geohashed_end_loc2')
...
def get_sloc_eloc(data,sample):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        order_eloc_dict = dict(zip(data['orderid'].values, data['geohashed_end_loc'].values))
        feat['sloc_eloc_count'] = feat['sloc_eloc_count'] - (feat['geohashed_end_loc'] == feat['orderid'].map(order_eloc_dict))
...
def get_bike_eloc(data,sample,candidate):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        feat[['biek_eloc_nloc_dis','biek_eloc_nloc_mdis']] = get_distance(feat,sloc='geohashed_end_loc',eloc='geohashed_end_loc2')
...
def get_user_sloc_eloc(data,sample,candidate):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        order_eloc_dict = dict(zip(data['orderid'].values, data['geohashed_end_loc'].values))
        feat['user_sloc_eloc_count'] = feat['user_sloc_eloc_count'] - (feat['geohashed_end_loc'] == feat['orderid'].map(order_eloc_dict))
...
# 时间×出发地
def get_time_sloc(data,sample):
    feat_path = cache_path + 'time_sloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
...
# 时间×目的地
def get_time_eloc(data,sample):
    feat_path = cache_path + 'time_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
...
# 时间×出发地×目的地
def get_time_sloc_eloc(data, sample):
    feat_path = cache_path + 'time_sloc_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        ...
        feat = pd.merge(sample, time_sloc_eloc_count, on=['time', 'geohashed_start_loc','geohashed_end_loc'], how='left').fillna(0)
        feat = feat['time_sloc_eloc_count']
        feat.to_hdf(feat_path, 'w', complib='blosc', complevel=5)
    return feat

# 用户×目的地×时间
def get_time_user_eloc(data,sample):
...
###############扩大统计范围##################
# 获取出发地热度（作为出发地的次数、人数）
def get_ex_sloc(data,sample):
    feat_path = cache_path + 'ex_sloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
...
# 获取目标地点的热度(目的地)
def get_ex_eloc(data,sample):
    feat_path = cache_path + 'ex_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        eloc_as_sloc = data.groupby('geohashed_start_loc', as_index=False)['userid'].agg(
...
# 用户×出发地（次数 目的地个数）
def get_ex_user_sloc(data,sample):
    feat_path = cache_path + 'ex_user_sloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
...
# 用户×目的地
def get_ex_user_eloc(data,sample):
    feat_path = cache_path + 'ex_user_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        user_eloc_as_sloc = data.groupby(['userid', 'geohashed_start_loc'], as_index=False)['userid'].agg(
...
# 出发地×目的地
def get_ex_sloc_eloc(data,sample):
    feat_path = cache_path + 'ex_sloc_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        sloc_eloc = data.groupby(['geohashed_start_loc','geohashed_end_loc'],as_index=False)['userid'].agg(
        ...
        near_loc = get_near_loc()
        feat = feat.merge(near_loc,left_on='geohashed_start_loc',right_on='loc',how='outer').fillna(0)
        feat['geohashed_start_loc'] = feat['near_loc']
        feat = feat.groupby(['geohashed_start_loc','geohashed_end_loc'],as_index=False).sum()
        feat = feat.merge(near_loc, left_on='geohashed_end_loc', right_on='loc', how='outer').fillna(0)
        feat['geohashed_end_loc'] = feat['near_loc']
        feat = feat.groupby(['geohashed_start_loc', 'geohashed_end_loc'], as_index=False).sum()
...
# 用户×出发地×目的地
def get_ex_user_sloc_eloc(data,sample):
    feat_path = cache_path + 'ex_user_sloc_eloc_%d.hdf' % (data['orderid'].sum() * sample['orderid'].sum())
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        user_sloc_eloc_count = data.groupby(['userid','geohashed_start_loc','geohashed_end_loc'],as_index=False)[
        ...
        feat = pd.merge(user_sloc_eloc_count,user_eloc_sloc_count,on=['userid','geohashed_start_loc','geohashed_end_loc'],how='outer').fillna(0)
        near_loc = get_near_loc()
        feat = feat.merge(near_loc, left_on='geohashed_start_loc', right_on='loc', how='outer').fillna(0)
...
def make_train_set(data,candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        # 扩大统计范围重新统计
        ex_sloc = get_ex_sloc(data,sample)                                  # 扩大 出发地
        ex_eloc = get_ex_eloc(data, sample)                                 # 获取目的地热度（作为目的地的个数、人数，作为出发地的个数、人数，折返比）
        ex_user_sloc = get_ex_user_sloc(data, sample)                       # 用户×出发地（次数 目的地个数）
        ex_user_eloc = get_ex_user_eloc(data, sample)                       # 用户×目的地
        ex_sloc_eloc = get_ex_sloc_eloc(data, sample)                       # 出发地×目的地
        ex_user_sloc_eloc = get_ex_user_sloc_eloc(data, sample)             # 用户×出发地×目的地
        gc.collect()
...
test_feat = test_feat1.append(test_feat2,ignore_index=True)
test_pred = test_feat[['orderid','geohashed_end_loc']].copy()
test_feat = test_feat[predictors].values
del data,test_data1,test_data2,test_feat1,test_feat2
gc.collect()
