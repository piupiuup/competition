...
def get_holiday_hour(data, sample, data_key):
    ...
    if os.path.exists(feat_path) & flag:
        ...
        hour_count = data.groupby(['hour'], as_index=False)['userid'].agg({'hour_count': 'count'})
        holiday_count = data.groupby(['holiday'], as_index=False)['userid'].agg({'holiday_count': 'count'})
        holiday_hour_count = data.groupby(['hour','holiday'],as_index=False)['userid'].agg({'holiday_hour_count': 'count'})
        ...
        feat = feat[['hour_count','holiday_count','holiday_hour_count']]
        feat.to_hdf(feat_path, 'w', complib='blosc', complevel=5)
    return feat
...
# 日期小时目的地
def get_date_hour_eloc(data, sample, data_key):
    feat_path = cache_path + 'date_hour_eloc_%d.hdf' % (data_key)
    if os.path.exists(feat_path) & flag:
        feat = pd.read_hdf(feat_path, 'w')
    else:
        data_temp = data.copy()
        ...
        data_hour_eloc_as_sloc = data_temp.groupby(['date', 'hour', 'geohashed_start_loc'], as_index=False)['userid'].agg({
            'data_hour_eloc_as_sloc': 'count'})
        data_hour_eloc_as_sloc.rename(columns={'geohashed_start_loc':'geohashed_end_loc'},inplace=True)
        feat = sample_temp.merge(data_hour_eloc_as_sloc, on=['date', 'hour', 'geohashed_end_loc'], how='left').fillna(0)
        order_sum_eloc = feat.groupby('orderid',as_index=False)['data_hour_eloc_as_sloc'].agg({'order_sum_eloc':'sum'})
        feat = feat.merge(order_sum_eloc,on='orderid',how='left')
        feat['data_hour_eloc_as_sloc_rate'] = feat['data_hour_eloc_as_sloc']/(feat['order_sum_eloc']+1)
        feat = feat[['data_hour_eloc_as_sloc','data_hour_eloc_as_sloc_rate']]
        feat.to_hdf(feat_path, 'w', complib='blosc', complevel=5)
    return feat
...
def second_feat(data, result):
    ...
    result['user_eloc_n_sloc'] = result['user_eloc_n_sloc'] - ((result['user_sloc_eloc_count'] == 0)
                                                               & ((result['geohashed_end_loc'] == result['orderid'].map(
        order_eloc_dict))))
    result['eloc_ex_rate'] =  result['eloc_count'] / result['ex_eloc_count']
    ...
    result['hour_eloc_count_hour_rate'] = result['hour_eloc_count'] / (result['hour_count'] + 0.001)
    ...
    result['holiday_hour_eloc_holiday_hour_rate'] = result['holiday_hour_eloc_count'] / (
    result['holiday_hour_count'] + 0.001)
    result['holiday_hour_eloc_sloc_holiday_hour_rate'] = result['holiday_hour_sloc_eloc_count'] / (
    result['holiday_hour_sloc_count'] + 0.001)
    ...
    result['eloc_exloc_rate'] = result['eloc_count'] / (result['exloc_count'] + 0.001)
    result['hour_eloc_exloc_rate'] = result['hour_eloc_count'] / (result['hour_exloc_count'] + 0.001)
    result['holiday_eloc_count_holiday_rate'] = result['holiday_eloc_count'] / (result['holiday_count'] + 0.001)
    result['holiday_eloc_exloc_rate'] = result['holiday_eloc_count'] / (result['holiday_exloc_count'] + 0.001)
    result['holiday_hour_eloc_holiday_hour_exloc_rate'] = result['holiday_hour_eloc_count'] / (
    result['holiday_hour_exloc_count'] + 0.001)
    result['holiday_hour_eloc_holiday_exloc_rate'] = result['holiday_hour_eloc_count'] / (
        result['holiday_eloc_count'] + 0.001)
    result['holiday_hour_eloc_hour_exloc_rate'] = result['holiday_hour_eloc_count'] / (
        result['hour_eloc_count'] + 0.001)
    return result
...
def make_train_set(data, candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        candidate_date = candidate['starttime'].str[:10].unique().tolist()
        data2 = data.copy()
        data2.loc[data2['starttime'].str[:10].isin(candidate_date), 'geohashed_end_loc'] = np.nan
        ...
        # 工作日× 地点
        holiday_hour = get_holiday_hour(data2, sample, data_key)                           # 工作日×时间
        holiday_eloc = get_holiday_eloc(data2, sample, data_key)                         # 工作日×时间×目的地
        holiday_hour_eloc = get_holiday_hour_eloc(data2, sample, data_key)               # 工作日×时间×目的地
        holiday_hour_sloc = get_holiday_hour_sloc(data2, sample, data_key)               # 工作日×时间×出发地
        holiday_hour_ex_sloc = get_holiday_hour_ex_sloc(data2, sample, data_key)         # 工作日×时间×扩大出发地
        holiday_exhour_ex_sloc = get_holiday_exhour_ex_sloc(data2, sample, data_key)     # 工作日×扩大时间×扩大出发地
        holiday_hour_sloc_eloc = get_holiday_hour_sloc_eloc(data2, sample, data_key)     # 工作日×时间×出发地×目的地
        holiday_hour_ex_sloc_eloc = get_holiday_hour_ex_sloc_eloc(data2, sample, data_key)  # 工作日×时间×扩大出发地×目的地
        holiday_exhour_ex_sloc_eloc = get_holiday_exhour_ex_sloc_eloc(data2, sample, data_key)  # 工作日×扩大时间×扩大出发地×目的地

        # # 工作日× 用户
        # holiday_hour_user = get_holiday_hour_user(data, sample, data_key)                         # 工作日×时间
        # holiday_user_eloc = get_holiday_user_eloc(data, sample, data_key)                         # 工作日×时间×目的地
        # holiday_hour_user_eloc = get_holiday_hour_user_eloc(data, sample, data_key)               # 工作日×时间×目的地
        # holiday_hour_user_sloc = get_holiday_hour_user_sloc(data, sample, data_key)               # 工作日×时间×出发地
        # holiday_hour_ex_user_sloc = get_holiday_hour_ex_user_sloc(data, sample, data_key)         # 工作日×时间×扩大出发地
        # holiday_exhour_ex_user_sloc = get_holiday_exhour_ex_user_sloc(data, sample, data_key)     # 工作日×扩大时间×扩大出发地
        # holiday_hour_user_sloc_eloc = get_holiday_hour_user_sloc_eloc(data, sample, data_key)     # 工作日×时间×出发地×目的地
        # holiday_hour_ex_user_sloc_eloc = get_holiday_hour_ex_user_sloc_eloc(data, sample, data_key)  # 工作日×时间×扩大出发地×目的地
        # holiday_exhour_ex_user_sloc_eloc = get_holiday_exhour_ex_user_sloc_eloc(data, sample, data_key)  # 工作日×扩大时间×扩大出发地×目的地


        # 区域特征
        exloc = get_exloc(data, sample, data_key)  # 区域
        # 日期小时目的地特征
        date_hour_eloc = get_date_hour_eloc(data, sample, data_key)
        ...
        result = concat([sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc,
                         ...
                         exhour_sloc_eloc, exhour_user_eloc, order_time,date_hour_eloc,
                         holiday_hour, holiday_eloc,exloc,
                         holiday_hour_eloc, holiday_hour_sloc,holiday_hour_ex_sloc,
                         holiday_exhour_ex_sloc, holiday_hour_sloc_eloc, holiday_hour_ex_sloc_eloc,
                         holiday_exhour_ex_sloc_eloc,
                         ])
        del sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc, bike_eloc, \
            ...
            order_time,date_hour_eloc,\
            holiday_hour, holiday_eloc, holiday_hour_eloc, exloc,\
            holiday_hour_sloc,holiday_hour_ex_sloc, holiday_exhour_ex_sloc, holiday_hour_sloc_eloc, \
...
predictors = train_feat.columns.drop(['orderid', 'geohashed_end_loc', 'label', 'userid',
                                      'bikeid', 'starttime', 'geohashed_start_loc','holiday_count',
                                      'holiday_hour_count'])
...
weight_dict = {}
weight = (train_feat['user_count']+1).map(weight_dict).values
lgb_train = lgb.Dataset(train_feat[predictors], train_feat.label,weight=weight,categorical_feature=['holiday','biketype'])
lgb_eval = lgb.Dataset(test_feat[predictors], test_feat.label,categorical_feature=['holiday','biketype'], reference=lgb_train)
