...
def get_all_order_pred(sample, data_key):
    ...
    if os.path.exists(result_path):
        ...
        result = result[['all_order_pred','sum_pred']]
...
def get_loc_to_loc(data, candidate, data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = pd.merge(candidate[['orderid', 'geohashed_start_loc']], sloc_eloc_count, on='geohashed_start_loc', how='left')
        order_eloc_dict = dict(zip(data['orderid'].values, data['geohashed_end_loc'].values))
...
def get_loc_with_loc(data, candidate, data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = pd.merge(candidate[['orderid', 'geohashed_start_loc']], sloc_eloc_2count,
...
def make_train_set(data, candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = concat([sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc,
                         ...
                         ex_sloc_eloc, ex_user_sloc_eloc,all_order_pred
                         ])
        del sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc, bike_eloc, \
            ...
            ex_user_sloc_eloc, all_order_pred
        gc.collect()
...
data = pd.concat([train1, train2, test])
del train, test
train_data = data[data['orderid']==4501483]
data = data[data['orderid']!=4501483]
train_feat = make_train_set(data, train_data)
...
data = pd.concat([train1, train2, test])
del train, test
data.loc[data['orderid']==i,'geohashed_end_loc'] = np.nan
train_data = data[data['orderid']==i]
train_feat_1501483 = make_train_set(data, train_data)