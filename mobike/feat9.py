...
selec_sample_path = r'C:\Users\csw\Desktop\python\mobike\data\select_sample.csv'
flag = True
...
del train, test
data_temp = data.copy()
orderid_sample = make_sample(data.orderid.tolist(),n_sub=6,seed=66)[0]
...
# bike_eloc = get_bike_next_loc(data, test_data, 1)
# test_pred = test_pred.merge(bike_eloc,on=['orderid','geohashed_end_loc'],how='left')
# test_pred = test_pred[~((~test_pred['bike_eloc_sep_time'].isnull())&(test_pred['label']==1))]
...
test.loc[:, 'geohashed_end_loc'] = np.nan
data = pd.concat([train, test])
user_count_dict1 = train.groupby('userid').size().to_dict()
user_count_dict2 = data.groupby('userid').size().to_dict()
data['user_count_dict1'] = data['userid'].map(user_count_dict1)
data['user_count_dict2'] = data['userid'].map(user_count_dict2)
orderid_1 = data[data['user_count_dict2']==1]['orderid'].tolist()

# 13号训练集
data_temp = data.copy()
data_temp.loc[data_temp['orderid'].isin(orderid_1), 'geohashed_end_loc'] = np.nan
train_data = data_temp[data_temp['orderid'].isin(orderid_1)].copy()
train_data = data_temp[data_temp['orderid'].isin(train['orderid'].values)].copy()
