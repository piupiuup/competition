...
def get_user_end_loc(data, candidate, data_key):
    ...
    if os.path.exists(result_path):
        ...
        test_pred_path = data_path + 'test_pred_{}.csv'.format(candidate['starttime'].values[0][8:10])
        test_pred = pd.read_csv(test_pred_path)
        data_temp = data[data['starttime'] >= '2017-05-25 00:00:00'].copy()
        data_temp = data_temp[(data_temp['starttime']>candidate['starttime'].max()) |
                              (data_temp['starttime']<candidate['starttime'].min())].copy()
        ...
        pred_user_eloc = data_temp.groupby(['userid', 'geohashed_end_loc'], as_index=False)['weight'].agg(
            {'pred_user_eloc': 'sum'})

        result = pd.merge(user_n_end_loc[['userid', 'geohashed_end_loc','user_eloc_count']],
                          pred_user_eloc[['userid', 'geohashed_end_loc','pred_user_eloc']],on=['userid', 'geohashed_end_loc'],how='outer').fillna(0)
        result['sum'] = result['user_eloc_count'] + result['pred_user_eloc']
        result1 = result[result['sum']>0.5].copy()
        result.sort_values('sum', inplace=True)
        result2 = result.groupby('userid').tail(6)
        result = pd.concat([result1,result2]).drop_duplicates()
        result = pd.merge(candidate[['orderid', 'userid']], result, on=['userid'], how='inner')
...
def get_loc_to_loc(data, candidate, data_key):
    ...
    if os.path.exists(result_path):
        ...
        data_temp = data_temp.merge(test_pred, on='orderid', how='inner')
        data_temp.rename(columns={'pred': 'weight'}, inplace=True)
        pred_sloc_eloc = data_temp.groupby(['geohashed_start_loc', 'geohashed_end_loc'], as_index=False)['weight'].agg(
            {'pred_sloc_eloc': 'sum'})

        result = pd.merge(sloc_eloc_count[['geohashed_start_loc', 'geohashed_end_loc', 'sloc_eloc_count']],
                          pred_sloc_eloc[['geohashed_start_loc', 'geohashed_end_loc', 'pred_sloc_eloc']],
                          on=['geohashed_start_loc', 'geohashed_end_loc'], how='outer').fillna(0)
        result['sum'] = result['sloc_eloc_count'] + result['pred_sloc_eloc']
        result = result[result['sum'] > 0.5].copy()
        result.sort_values('sum', inplace=True)
        result = result.groupby('geohashed_start_loc').tail(7)
...
def get_user_bike(data, sample, data_key):
    ...
    if os.path.exists(result_path) & flag:
        ...
        test_temp = data[data['geohashed_end_loc'].isnull()]
        bike_next_loc = get_bike_next_loc(test_temp, test_temp, data_key)
...
def make_train_set(data, candidate):
    ...
    if os.path.exists(result_path) & flag:
        ...
        result = concat([sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc,
                         ...
                         holiday_hour_eloc,holiday_sloc_eloc,test_pred,
        ...
        del sample, user_count, start_loc, end_loc, user_sloc, user_eloc, sloc_eloc, \
            ...
            holiday_exhour_ex_sloc, holiday_hour_sloc_eloc,test_pred,\
...
# 获取周围一圈的地点
def get_near_nloc(result_sub,new_loc):
    result_sub_temp = result_sub.copy()
    for loc in new_loc:
        result_sub = (set(geohash.neighbors(loc)) | result_sub)
    new_loc = result_sub - result_sub_temp
    return result_sub,new_loc

# 周围的（2n+1）×（2n+1）个地点
def get_near_loc_dict_n(n):
    result_path = cache_path + 'near_loc_{}.hdf'.format(n)
    if os.path.exists(result_path):
        result = pickle.load(open(result_path, 'rb+'))
    else:
        import geohash
        ...
        loc_set = train['geohashed_start_loc'].tolist() \
                   ...
                   + test['geohashed_start_loc'].tolist()
        loc_set = set(loc_set)
        if np.nan in loc_set:
            loc_set.remove(np.nan)
        result = {}
        for loc in loc_set:
            result_sub = set([loc])
            new_loc = set([loc])
            for i in range(n):
                result_sub,new_loc = get_near_nloc(result_sub,new_loc)
            result[loc] = list(result_sub & loc_set)
        pickle.dump(result, open(result_path, 'wb+'))
    return result

# 周围周围的（2n+1）×（2n+1）个地点中top10的地点
def get_hot_loc_n(data,sample,m=10):
    '''
    :param data: 提取特征用的数据
    :param sample: 样本
    :param n: 提取前n个
    :return: DataFrame，columns=['orderid','geohashed_end_loc']
    '''
    eloc_count = data.groupby('geohashed_end_loc',as_index=False)['geohashed_end_loc'].agg({'eloc_count':'count'})
    eloc_count.sort_values('eloc_count',inplace=True)
    sloc_header_dict = {}
    for sloc in sample['geohashed_start_loc'].unique():
        sloc_header_dict[sloc] = []
    near_loc_dict_n = get_near_loc_dict_n(9)
    for eloc in tqdm.tqdm(eloc_count['geohashed_end_loc'].values):
        for near_loc in near_loc_dict_n[eloc]:
            if (near_loc in sloc_header_dict):
                if len(sloc_header_dict[near_loc]) < m:
                    sloc_header_dict[near_loc].append(near_loc)
    return sloc_header_dict
