...
def get_rank_feat(data,data_key):
    ...
    if os.path.exists(result_path) & (not inplace):
        ...
        for key in [['ip', 'device', 'os'],['ip', 'device', 'os', 'app'],
            ...
            data_temp['_'.join(key) + '_rank1'] = group_rank(data_temp, key, data_key,ascending=True)
            data_temp['_'.join(key) + '_rank-1'] = group_rank(data_temp, key,data_key, ascending=False)
