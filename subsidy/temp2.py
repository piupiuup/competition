...
def get_feature(mold):
    ...
    card_data['day'] = card_data['消费时间'].apply(lambda x: x.date())
    #读取院系和成绩信息
    ...
    #卡充值次数
    card_data_temp = card_data[card_data['消费类别'].isin(['圈存转账','卡充值'])]
    ...
    #每个同学开水消费总次数
    data['count_of_meal'] = card_data_temp[card_data_temp['消费方式']=='开水'].groupby('studentid')['消费金额'].agg('count')
    #每个同学开水消费平均值
    ...
    #清理校车异常数据
    card_data_temp = card_data[~card_data['消费地点'].isin(['loc91'])]
    #每个同学校车消费总额
    data['sum_of_bus'] = card_data_temp[card_data_temp['消费方式']=='校车'].groupby('studentid')['消费金额'].agg('sum')
    #每个同学校车消费总次数
    data['count_of_bus'] = card_data_temp[card_data_temp['消费方式']=='校车'].groupby('studentid')['消费金额'].agg('count')
    #每个同学校车消费平均值
    data['mean_of_bus'] = card_data_temp[card_data_temp['消费方式']=='校车'].groupby('studentid')['消费金额'].agg('mean')
    #校车消费的天数
    data['day_of_bus'] = card_data_temp[card_data_temp['消费方式']=='校车'].groupby('studentid')['day'].agg('nunique')
    ...
    # 每个同学超市消费最小值
    data['min_of_shopping'] = card_data[card_data['消费方式'] == '超市'].groupby('studentid')['消费金额'].agg('min')
    #超市消费的天数
    ...
    #计算第二年校园卡消费比第一年增加量
    data['differenct_of_consume'] = data_temp['sum_of_consume']-data['sum_of_consume']
    #计算第二年校园卡消费比第一年增加量所占百分比
    data['percent_differenct_consume'] = data['differenct_of_consume'] /(data_temp3['sum_of_consume']+1)
    #计算第二年校园卡食堂消费比第一年增加量
    data['differenct_of_meal'] = data_temp['sum_of_meal']-data['sum_of_meal']
    #计算第二年校园卡食堂消费比第一年增加量所占百分比
    data['percent_differenct_meal'] = data['differenct_of_meal'] /(data_temp3['sum_of_meal']+1)
    #计算第二年校园卡非食堂消费比第一年增加量
    data['differenct_of_nomeal'] = data['differenct_of_consume']-data['differenct_of_meal']
    #计算第二年校园卡非消费比第一年增加量所占百分比
    data['percent_differenct_noconsume'] = data['differenct_of_nomeal'] /(data_temp3['sum_of_consume']-data_temp3['sum_of_meal']+1)
    ...
    location = card_data.groupby(['studentid','消费地点'])['studentid'].agg('count').unstack()
