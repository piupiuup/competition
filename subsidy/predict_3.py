...
#删除id>22073,训练，预测
def predict_3(train,test,alg=XGBClassifier,coefficient=1,silent=1):
    ...
    train_11 = train_1[train_1['studentid']<22073]

    train_2= train.copy()
    test_2 = test.copy()
    train_2 = train_2[train['money']>0]
    ...
    alg1 = alg()
    alg1.fit(train_11[predictors], train_11[target])
    result1 = alg1.predict_proba(test_1[predictors])
    n1000 = int(count1000(result1[:, 1]) * coefficient)
    if silent == 0: print '大于0的个数为：', n1000
    result1 = pd.DataFrame({'studentid': test_1['studentid'].values, 1: result1[:, 1]}).sort_values(by=[1],
                                                                                                    ascending=False)
