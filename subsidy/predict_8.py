...
#四次新二分类
def predict_8(train, test, silent=1):
    ...
    alg1 = GradientBoostingClassifier(learning_rate=0.1,n_estimators=60, max_depth=4,min_samples_leaf=6,min_samples_split=150,subsample=1)
    alg1.fit(train_1[predictors], train_1[target])
    result1 = alg1.predict_proba(test[predictors])

    # model of select id which equal 1000
    alg2 = GradientBoostingClassifier(learning_rate=0.1,n_estimators=60, max_depth=4,min_samples_leaf=6,min_samples_split=150,subsample=1)
    alg2.fit(train_2[predictors], train_2[target])
    result2 = alg2.predict_proba(test[predictors])

    # model of select id which equal 1500
    alg3 = GradientBoostingClassifier(learning_rate=0.1,n_estimators=60, max_depth=4,min_samples_leaf=6,min_samples_split=150,subsample=1)
    alg3.fit(train_3[predictors], train_3[target])
    result3 = alg3.predict_proba(test[predictors])

    # model of select id which equal 2000
    alg4 = GradientBoostingClassifier(learning_rate=0.1, n_estimators=60, max_depth=4, min_samples_leaf=6,min_samples_split=150, subsample=1)
