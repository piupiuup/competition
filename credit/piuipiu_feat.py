...
def get_previous_applications_feat(ids,data_key):
    ...
    if os.path.exists(result_path) & 1:
        ...
        num_aggregations = {
            ...
            'DAYS_DECISION_diff': ['min', 'max', 'mean', 'std'],
        ...
        prev_agg.columns = pd.Index(
            [e[0] + "_" + e[1] + '{}days'.format(3000) for e in prev_agg.columns.tolist()])
        ...
        for days in [1300, 580, 280, 90]:
            ...
            prev_agg.columns = pd.Index(
                [e[0] + "_" + e[1] + '{}days'.format(days) for e in prev_agg.columns.tolist()])
            data = data.merge(prev_agg.reset_index(), how='left', on='SK_ID_CURR')
