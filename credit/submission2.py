...
for name in ['app','bureau','pcb','prev','ccb','ins','all']:
    oof_preds = np.zeros(train_df.shape[0])
    sub_preds = np.zeros(test_df.shape[0])
    print('{} features training.....................................................'.format(name))
    if name == 'all':
        predictors = [c for c in data.columns if c not in ['label', 'SK_ID_CURR', 'index']]
    else:
        predictors = [c for c in data.columns if (c not in ['label','SK_ID_CURR','index']) & (c.startswith(name+'_'))]
