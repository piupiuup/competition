...
# 构造样本集
def get_candicate(prior,user_order,eval):
    df_path = r'F:\cache\instacart_cache\%s_candicate.hdf' % eval
    if os.path.exists(df_path) & 1:
        df = pd.read_hdf(df_path, 'w')
    else:
...
def make_train_set(eval):
    df_path = r'F:\cache\instacart_cache\%s_set.hdf' % eval
    if os.path.exists(df_path) & 1:
        df = pd.read_hdf(df_path, 'w')
    else:
        prior = get_prior()
...
# 线下调参
features = ['product_id', 'order_dow', 'order_hour_of_day',
       ...
       'order_train_aisle_rebuy_rate',
       'order_train_department_rebuy_rate']

eval_train = df_train[:int(df_train.shape[0]*0.7)]
eval_test = df_train[int(df_train.shape[0]*0.7):]

lgb_train = lgb.Dataset(eval_train[features],eval_train.label)
lgb_eval = lgb.Dataset(eval_test[features],eval_test.label)
params = {
    'task': 'train',
    'boosting_type': 'gbdt',
    'objective': 'binary',
...
preds = gbm.predict(eval_test[features])
