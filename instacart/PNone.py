...
user_test_label = model.predict(xgb.DMatrix(user_test[predictors]))
user_test_label = pd.DataFrame({'user_id':user_test['user_id'].values,'label':user_test_label})
...
'''
import pickle
user_test_nlabel_path = r'F:\cache\instacart_cache\user_test_nlabel.pkl'
if os.path.exists(user_test_nlabel_path):
    user_test_nlabel = pickle.load(open(user_test_nlabel_path, 'rb+'))
else:
    pickle.dump(user_test_nlabel, open(user_test_nlabel_path, 'wb+'))
'''
...
def right_test(train,predictors,target,test=None):
    ...
    pred_train = train[['user_id']]
    pred_train['pred_nlabel'] = 0

    if test is not None:
        test_X = test[predictors]
        pred_test = test[['user_id']]
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        ## build xgb
        xgtrain = xgb.DMatrix(sub_train_X, sub_train_y)
        xgtest = xgb.DMatrix(sub_test_X, sub_test_y)
        model = xgb.train(params, xgtrain, 870)
        pred_train['pred_nlabel'].iloc[test_index] = model.predict(xgtest)
...
order_train_pred_nlabel,order_test_pred_nlabel = right_test(user_train,predictors,'nlabel',user_test)
user_pred_n_label = pd.concat([order_train_pred_nlabel,order_train_pred_nlabel])
dump(user_pred_n_label,'user_pred_n_label')


# 再给特征添加特征
user_pred_n_label = load('user_pred_n_label')
user_train = pd.merge(user_train, user_pred_n_label,                    on='user_id', how='left')
user_test = pd.merge(user_test, user_pred_n_label,                      on='user_id', how='left')



# 调参用
eval_user_train = user_train[:int(user_train.shape[0]*0.7)]
eval_user_test = user_train[int(user_train.shape[0]*0.7):]
import xgboost
xgb_train = xgboost.DMatrix(eval_user_train[predictors],eval_user_train.label)
xgb_eval = xgboost.DMatrix(eval_user_test[predictors],eval_user_test.label)
...
xgb_train = xgb.DMatrix(user_train[predictors],user_train['label'])
...
def right_test(train,predictors,target,test=None):
    ...
    pred_test = test[['order_id']]
    pred_test['reorder_num'] = 0
    pred_train = train[['order_id']]
    pred_train['reorder_num'] = 0
    kf = KFold(len(train_y), n_folds = 5, shuffle=True, random_state=520)
    for i, (train_index, test_index) in enumerate(kf):

        ## build xgb
        xgb_train = xgb.DMatrix(train_X.iloc[train_index], train_y.iloc[train_index])
        xgb_test = xgb.DMatrix(train_X.iloc[test_index], train_y.iloc[test_index])
        watchlist = [(xgb_test, "test")]
        bst = xgboost.train(params=xgb_params,
                            dtrain=xgb_train,
                            num_boost_round=5000,
                            evals=watchlist,
        ...
        model = xgb.train(xgb_params, xgb_train, bst.best_iteration)
        pred_train['reorder_num'].iloc[test_index] = model.predict(xgb_test)


        pred_test['reorder_num'] += model.predict(xgb.DMatrix(test_X))
    print ('Done in %.1fs!' % (time.time()-t0))
    pred_test['reorder_num'] = pred_test['reorder_num']/5
    return (pred_train,pred_test)

order_train_none,order_test_none = right_test(user_train,predictors,'label',user_test)
order_none = pd.concat([order_train_none,order_test_none])
order_none.to_csv(r'/home/user/Desktop/cuishiwen/order_none.csv',index=False)
