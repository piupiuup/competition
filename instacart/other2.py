...
path_data = r'C:\Users\csw\Desktop\python\instacart\data\\'
...
agg_dict_3 = {'reordered':
                  {'_user_reorder_equal':lambda x: sum(x == 1)},
              'order_number':
                   {'_user_reorder_big_1':lambda x: sum(x > 1)},
              'product_id': {'_user_total_products': 'count',
                             '_user_distinct_products': lambda x: x.nunique()}}
us = ka_add_groupby_features_1_vs_n(priors_orders_detail, ['user_id'], agg_dict_3)
us['_user_reorder_ratio'] = us['_user_reorder_equal']/us['_user_reorder_big_1']
...
data.loc[:, 'reordered'] = data.reordered.fillna(0)
df_train = data.loc[data.eval_set == "train", :]

train = df_train[:int(df_train.shape[0]*0.7)]
test = df_train[int(df_train.shape[0]*0.7):]


features = [ '_up_order_count', '_up_first_order_number',
       ...
       '_prod_reorder_ratio', '_prod_reorder_times', '_user_total_orders',
       '_user_sum_days_since_prior_order', '_user_mean_days_since_prior_order',
       '_user_reorder_ratio', '_user_total_products',
       '_user_distinct_products', '_user_average_basket',
        'time_since_last_order', '_up_order_rate',
       '_up_order_since_last_order', '_up_order_rate_since_first_order',
        '_user_reorder_equal','_user_reorder_big_1'
       ]


lgb_train = lgb.Dataset(train[features],train.reordered)
lgb_eval = lgb.Dataset(test[features],test.reordered, reference=lgb_train)
params = {
    'task': 'train',
    'boosting_type': 'gbdt',
    'objective': 'binary',
