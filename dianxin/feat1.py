...
def pre_treatment(data,data_key):
    ...
    if os.path.exists(result_path) & 0:
        ...
        data['former_complaint_fee_100'] = ((data[c] % 100 == 0) & (data[c] % 500 != 0) & (data[c] % 3000 != 0) & (data[c] != 0))
        data['former_complaint_fee_30'] = ((data[c] % 30 == 0) & (data[c] % 500 != 0) & (data[c] % 3000 != 0) & (data[c] != 0))
        ...
        fee1_pred = pd.read_csv(r'C:\Users\cui\Desktop\python\dianxin\submission\fee1_pred.csv')
        data = data.merge(fee1_pred, on='user_id', how='left')
        data['fee1_diff'] = data['1_total_fee'] - data['fee1_pred']
        last_month_traffic_pred = pd.read_csv(
            r'C:\Users\cui\Desktop\python\dianxin\submission\last_month_traffic_pred.csv')
        data = data.merge(last_month_traffic_pred, on='user_id', how='left')
        data['last_month_traffic_pred_diff'] = data['last_month_traffic'] - data['last_month_traffic_pred']
        month_traffic_last_month_traffic_sum_pred = pd.read_csv(
            r'C:\Users\cui\Desktop\python\dianxin\submission\month_traffic_last_month_traffic_sum_pred.csv')
        data = data.merge(month_traffic_last_month_traffic_sum_pred, on='user_id', how='left')
        data['month_traffic_last_month_traffic_sum_pred_diff'] = data['month_traffic_last_month_traffic_sum'] - data[
            'month_traffic_last_month_traffic_sum_pred']
        del data['fee1_pred'], data['last_month_traffic_pred'], data['month_traffic_last_month_traffic_sum_pred']
...
def get_apred_feat(data,data_key):
    ...
    if os.path.exists(result_path) & 0:
        ...
        apred1 = pd.read_csv(r'C:\Users\cui\Desktop\python\dianxin\submission\1\train_pred_0.78004_0.1698.csv')     # most old a+
        feat = feat.merge(apred1,on='user_id',how='left')
        # feat.to_feather(result_path)
    return feat
...
def make_feat(data,data_key):
    ...
    if os.path.exists(result_path) & 0:
        ...
        result = [data]
        # print('开始构造特征...')
        result.append(get_max_feat(data,data_key))     # max特征
        # result.append(get_user_feat())     # 用户特征
        # result.append(get_item_feat())     # 商品特征
        result.append(get_apred_feat(data,data_key))     # 商店特征
