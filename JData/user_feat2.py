...
def get_cycle(get_function, end_date, key=['user_id'], n=6):
    ...
    '''
    dump_path = r'F:\cache2\_%s_%s.pkl' % (get_function.__name__, end_date)
...
def get_cycle2(get_function, end_date, key=['user_id']):
    ...
    key：  为不同日期区间提取特征merge在一起是用到的键
    '''
    dump_path = r'F:\cache2\cycle2_%s_%s.pkl' % (get_function.__name__, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = None
...
def get_actions(start_date=None, end_date=None):
    if start_date==None:
        dump_path = r'F:\cache2\all_action.pkl'
        if os.path.exists(dump_path):
            actions = pickle.load(open(dump_path, 'rb+'))
        else:
...
def get_cate8(start_date, end_date):
    dump_path = r'F:\cache2\cate8.pkl'
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
#读取cate8中回头看到的数据
def get_back_cate8():
    dump_path = r'F:\cache2\back_cate8.pkl'
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
...
def get_other(start_date, end_date):
    dump_path = r'F:\cache2\cate_other.pkl'
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
def get_other_conversion(start_date, end_date):
    dump_path = r'F:\cache2\other_conversion_actions.pkl'
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
def get_cate8_conversion(start_date, end_date):
    dump_path = r'F:\cache2\cate8_conversion_actions.pkl'
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
def get_ID(end_date,i):
    dump_path = r'F:\cache2\ID_%ddays_%s.pkl' % (i,end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
...
def get_user_id(end_date, i):
    dump_path = r'F:\cache2\user_id_%s_%ddays.pkl' % (end_date,i)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = datetime.strptime(end_date, '%Y-%m-%d') - timedelta(days=i)
...
def get_basic_user_feat(end_date):
    dump_path = r'F:\cache2\basic_user_%s.pkl' % (end_date)
    if os.path.exists(dump_path):
...
def get_user_tm(start_date,end_date):
    dump_path = r'F:\cache2\user_tm_%s_%s.pkl' % (start_date,end_date)
    if os.path.exists(dump_path):
        ...
        user_first_tm['user_first_tm'] = user_first_tm['time'].map(lambda x: diff_of_minutes(end_date, x))
        user_last_tm2['user_last_tm'] = user_last_tm2['time'].map(lambda x: diff_of_minutes(end_date, x))
        user_tm = pd.merge(user_first_tm, user_last_tm2, on='user_id', how='left')
        ...
        user_tm = user_tm[['user_id', 'user_last_tm', 'user_first_tm', 'user_sep_tm','if_delect','first_tm_of_hour']]
        pickle.dump(user_tm, open(dump_path, 'wb+'))
    return user_tm

# 购买一个物品平均使用天数
def get_user_use_day(end_date,n_days):
    dump_path = r'F:\cache2\user_use_day_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        ...
        use_day['user_use_day'] = use_day.apply(lambda x:diff_of_days(x['buy_tm'],x['first_tm']),axis=1)
...
def get_user_maxday_tm(end_date,n_days):
    dump_path = r'F:\cache2\user_maxday_tm_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_most_active_hour(end_date,n_days):
    dump_path = r'F:\cache2\user_most_active_hour_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_type4(start_date,train_end_date):
    dump_path = r'F:\cache2\user_type4_%s_%s.pkl' % (start_date, train_end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
...
def get_user_other_type4(end_date, n_days):
    dump_path = r'F:\cache2\user_other_type4_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        n_days = n_days
...
def get_user_n_sku_acc(end_date, n_days):
    dump_path = r'F:\cache2\user_n_sku_acc_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        expoent1 = 0.99
...
def get_user_type2_del_type3(end_date, n_days):
    dump_path = r'F:\cache2\user_type2_del_type3_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        n_days = n_days
        start_date = date_add_days(end_date, -n_days)
        actions = get_cate8(start_date, end_date)
        actions = actions[actions['type'].isin([2,3])].drop_duplicates(['user_id', 'sku_id'], keep='last')
        actions = actions.groupby('user_id',as_index=False)['type'].agg({'type2_del_type3':'count'})
        other = get_other(start_date, end_date)
        other = other[other['type'].isin([2, 3])].drop_duplicates(['user_id', 'sku_id'], keep='last')
        other = other.groupby('user_id', as_index=False)['type'].agg({'type2_del_type3_other': 'count'})
        actions = pd.merge(actions,other,on='user_id',how='outer').fillna(0)
        actions['percent_of_cate8_type2'] = actions.apply(lambda x: x['type2_del_type3']/(x['type2_del_type3']+x['type2_del_type3_other']),axis=1)
        actions = actions[['user_id','type2_del_type3','type2_del_type3_other','percent_of_cate8_type2']]
        pickle.dump(actions, open(dump_path, 'wb+'))
    return actions
...
def get_user_model(end_date, n_days):
    dump_path = r'F:\cache2\user_model_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        n_days = n_days
...
def get_user_max_sku(end_date, n_days):
    dump_path = r'F:\cache2\user_max_sku_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        ...
        max_sku = max_sku.groupby('user_id',as_index=False)['n_actions'].agg({'user_max_sku':'max','user_std_sku':'std','user_gini_sku':get_gini})
        pickle.dump(max_sku, open(dump_path, 'wb+'))
    return max_sku

#用户最后一次浏览行为
def get_user_last_type(end_date, n_days):
    dump_path = r'F:\cache2\user_last_type_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_type_tm(start_date,train_end_date):
    dump_path = r'F:\cache2\user_type_tm_%s_%s.pkl' % (start_date, train_end_date)
    if os.path.exists(dump_path):
...
def get_user_type_conversion(end_date,n_days):
    dump_path = r'F:\cache2\user_type_conversion_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        ...
        columns = ['user_id', 'user_1_conversion', 'user_2_conversion', 'user_3_conversion',
                   ...
                   'count_of_other_action']
        percent = percent[columns]
        percent['all_other_action'] = percent.apply(lambda x:0 if x['count_of_other_action']>0 else x['count_of_other_action'],axis=1)
        percent.rename(columns={'action_4':'other_4'},inplace=True)
...
def get_user_sku_conversion(end_date,n_days):
    dump_path = r'F:\cache2\user_sku_conversion_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        percent = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_cate_conversion(end_date,n_days):
    dump_path = r'F:\cache2\user_cate_conversion_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        percent = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_other_action_acc(end_date, n_days):
    dump_path = r'F:\cache2\user_other_action_acc_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_percent_of_cate8_acc(end_date, n_days):
    dump_path = r'F:\cache2\user_percent_of_cate8_acc_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def get_user_action(start_date, end_date, i):
    dump_path = r'F:\cache2\user_action_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
...
def get_user_all_action(start_date, end_date, i):
    dump_path = r'F:\cache2\user_all_action_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
...
def get_user_type1_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type1_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        expoent = 0.59
...
def get_user_type2_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type2_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.005
...
def get_user_type3_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type3_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.00003
...
def get_user_type5_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type5_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.00003
...
def get_user_type6_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type6_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.005
...
def get_user_type1type6_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_type1type6_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.2
...
def get_user_action_feat(start_date, end_date, i):
    dump_path = r'F:\cache2\user_action_accumulate_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
...
def get_user_other_feat(start_date, end_date, i):
    dump_path = r'F:\cache2\user_other_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_other(start_date, end_date)
...
def get_user_type_sku(start_date, end_date, i):
    dump_path = r'F:\cache2\user_type_n_sku_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
...
def get_user_n_model(start_date, end_date, i):
    dump_path = r'F:\cache2\user_n_model_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        user_type = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
...
def get_user_n_model_acc(end_date, n_days):
    dump_path = r'F:\cache2\user_n_model_acc_%s_%ddays.pkl' % (end_date, n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        expoent1 = 0.99
...
def get_user_active_minutes(start_date, end_date, i):
    dump_path = r'F:\cache2\user_action_minutes_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date,end_date)
...
def get_user_active_minutes_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_action_minutes_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.006
...
def get_user_active_hours_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_action_hours_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        coe = 0.005
...
def get_user_active_days(start_date, end_date, i):
    dump_path = r'F:\cache2\user_action_days_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date,end_date)
...
def get_user_active_days_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_action_days_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        expoent = 0.995
...
def get_user_active_weeks_acc(start_date, end_date):
    dump_path = r'F:\cache2\user_action_weeks_acc_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        expoent = 0.81
...
def get_user_other_tm(end_date,n_days):
    dump_path = r'F:\cache2\user_other_tm_%s_%ddays.pkl' % (end_date,n_days)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        start_date = date_add_days(end_date, -n_days)
...
def ger_user_if_buy_other(start_date,end_date):
    dump_path = r'F:\cache2\user_if_buy_other_tm_%s.pkl' % end_date
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_actions(start_date,end_date)
...
def get_action_back(start_date, end_date, i):
    dump_path = r'F:\cache2\action_back_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
...
def get_user_accumulate(start_date, end_date):
    dump_path = r'F:\cache2\user_accumulate_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
...
def get_labels(start_date, end_date):
    dump_path = r'F:\cache2\labels_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
...
def make_user_train_set(train_end_date, test_start_date, test_end_date):
    dump_path = r'F:\cache2\user_train_set_%s.pkl' % (train_end_date)
    if os.path.exists(dump_path) & 1:
        ...
        user_max_sku = get_user_max_sku(train_end_date,n_days=30)                   # 用户浏览最多的一个商品个数
