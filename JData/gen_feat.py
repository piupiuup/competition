...
def get_user_type_conversion(start_date, end_date):
    dump_path = r'F:\cache\user_type_conversion_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        percent = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_conversion(start_date, end_date)
...
#用户sku数转化率
def get_user_sku_conversion(start_date, end_date):
    dump_path = r'F:\cache\user_sku_conversion_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        percent = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_conversion(start_date, end_date)
...
#用户cate数转化率
def get_user_cate_conversion(start_date, end_date):
    dump_path = r'F:\cache\user_cate_conversion_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        percent = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_conversion(start_date, end_date)
...
def make_train_set(train_end_date, test_start_date, test_end_date):
    ...
    if os.path.exists(dump_path) & 1:
        ...
        user_type_conversion = get_user_type_conversion(start_date, train_end_date) # 用户转化率
        ...
        user_action = get_cycle(get_user_action, train_end_date, 'user_id')    # 用户行为次数特征
...
def make_user_train_set(train_end_date, test_start_date, test_end_date):
    ...
    if os.path.exists(dump_path) & 1:
        ...
        start_date = "2016-01-31"
        user_id = get_user_id(train_end_date, 5)                                    # 提取用户id
        user = get_basic_user_feat(train_end_date)                                  # 用户基本信息
        user_type_conversion = get_user_type_conversion(start_date, train_end_date) # 用户type转化率
        user_sku_conversion = get_user_sku_conversion(start_date, train_end_date)   # 用户浏览物品数转化率
        user_cate_conversion = get_user_cate_conversion(start_date, train_end_date) # 用户cate转化率
        user_tm = get_user_tm(start_date, train_end_date)                           # 用户浏览cate8时间
        ...
        user_last_type = get_user_last_type(train_end_date,n_days=30)               # 用户最后一次浏览行为
        #user_other_type4 = get_user_other_type4(train_end_date,30)                  # 用户购买购买过多少其他商品
        user_last_buy_tm = get_user_last_buy_tm(start_date,train_end_date)          # 用户最后购买非cate8的最后时间
        #user_if_buy_other = ger_user_if_buy_other(start_date,train_end_date)        # 用户最后一次浏览cate8之后是否购买过其他商品
        #user_type_tm = get_user_type_tm(start_date, train_end_date)                 # 用户浏览（type）行为事件
        labels = get_labels(test_start_date, test_end_date)                         # 获取label
        ...
        user_type1 = get_user_type1_acc(start_date, train_end_date)
        user_type2 = get_user_type2_acc(start_date, train_end_date)
        user_type3 = get_user_type3_acc(start_date, train_end_date)
        user_type5 = get_user_type5_acc(start_date, train_end_date)
        user_type6 = get_user_type6_acc(start_date, train_end_date)
        user_type1type6 = get_user_type1type6_acc(start_date, train_end_date)
        ...
        #user_type_n_sku = get_cycle(get_user_type_sku, train_end_date, 'user_id')           # 用户行为种类对应的商品个数
        #user_n_model = get_cycle(get_user_n_model, train_end_date, 'user_id')               # 用户的model种类数
        user_n_model = get_user_n_model_acc(train_end_date, 30)


        user = pd.merge(user_id, user,              how='left', on='user_id')
        user = pd.merge(user, user_type1,           how='left', on='user_id')
        user = pd.merge(user, user_type2,           how='left', on='user_id')
        user = pd.merge(user, user_type3,           how='left', on='user_id')
        user = pd.merge(user, user_type5,           how='left', on='user_id')
        user = pd.merge(user, user_type6,           how='left', on='user_id')
        user = pd.merge(user, user_type1type6,      how='left', on='user_id')
        user = pd.merge(user, user_type_conversion, how='left', on='user_id')
