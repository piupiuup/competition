...
def get_cate8(start_date, end_date):
    dump_path = r'F:\cache\cate8_2016-01-31_2016-04-16.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
def get_other(start_date, end_date):
    dump_path = r'F:\cache\cate_other_2016-01-31_2016-04-16.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        actions = pickle.load(open(dump_path, 'rb+'))
        actions = actions[(actions.time >= start_date) & (actions.time < end_date)]
    else:
...
def get_basic_user_feat(end_date):
    ...
    if os.path.exists(dump_path):
        ...
        user['user_reg_tm'] = pd.DataFrame(user['user_reg_tm'].apply(lambda x:diff_of_days1(end_date,x)))
...
#用户点击种类数
def get_user_n_model(start_date, end_date):
    dump_path = r'F:\cache\user_n_model_%s_%s.pkl' % (start_date, end_date)
    if os.path.exists(dump_path):
        user_n_model = pickle.load(open(dump_path, 'rb+'))
    else:
        actions = get_cate8(start_date, end_date)
        actions.drop_duplicates(['user_id','model_id'],inplace=True)
        user_n_model = actions.groupby('user_id',as_index=False)['model_id'].count()
        user_n_model.rename(columns={'model_id':'n_model'},inplace=True)
        pickle.dump(user_n_model, open(dump_path, 'wb+'))
    return user_n_model
...
def make_train_set(train_end_date, test_start_date, test_end_date, days=30):
    ...
    if os.path.exists(dump_path) & 1:
        ...
        #user_n_model = get_user_n_model(start_date, train_end_date)                 # 用户model种类数
        ...
        user_action = None                                                          # 用户行为次数特征
        user_all_action = None                                                      # 用户所有行为次数分析
        user_active_days = None                                                     # 用户活跃天数
        #user_active_minutes = None                                                  # 用户活跃分钟数
        user_other = None                                                           # 用户近期浏览其他商品的次数
        actions = None                                                              # 交互行为次数特征
        active_days = None                                                          # 交互活跃天数
        #active_minutes = None                                                       # 交互活跃分钟数

        for i in (1, 2, 3, 5, 7, 10, 15, 21, 30, 60):
            start_date = date_add_days(train_end_date,-i)
            if actions is None:
                user_action = get_user_action_feat(start_date, train_end_date, i)
                #user_all_action = get_user_all_action_feat(start_date, train_end_date, i)
                user_active_days = get_user_active_days(start_date, train_end_date, i)
                #user_active_minutes = get_user_active_minutes(start_date, train_end_date, i)
                user_other = get_user_other_feat(start_date, train_end_date, i)
                actions = get_action_feat(start_date, train_end_date, i)
                active_days = get_active_days(start_date, train_end_date, i)
                #active_minutes = get_active_minutes(start_date, train_end_date, i)
            else:
                if (i<6):
                    user_action = pd.merge(get_user_action_feat(start_date, train_end_date, i), user_action, how='left',
                                        on=['user_id'])
                    #user_all_action = pd.merge(get_user_all_action_feat(start_date, train_end_date, i), user_all_action, how='left',
                    #                    on=['user_id'])
                    user_active_days = pd.merge(get_user_active_days(start_date, train_end_date, i), user_active_days,how='left',
                                        on=['user_id'])
                    #user_active_minutes = pd.merge(get_user_active_minutes(start_date, train_end_date, i), user_active_minutes,how='left',
                    #                    on=['user_id'])
                    user_other = pd.merge(get_user_other_feat(start_date, train_end_date, i), user_other, how='left',
                                        on=['user_id'])
                    actions = pd.merge(get_action_feat(start_date, train_end_date, i), actions, how='left',
                                        on=['user_id', 'sku_id'])
                    active_days = pd.merge(get_active_days(start_date, train_end_date, i), active_days, how='left',
                                        on=['user_id', 'sku_id'])
                    #active_minutes = pd.merge(get_active_minutes(start_date, train_end_date, i),active_minutes, how='left',
                    #                    on=['user_id', 'sku_id'])
                else:
                    user_action = pd.merge(user_action, get_user_action_feat(start_date, train_end_date, i), how='left',
                                        on=['user_id'])
                    #user_all_action = pd.merge(user_all_action, get_user_all_action_feat(start_date, train_end_date, i), how='left',
                    #                    on=['user_id'])
                    user_active_days = pd.merge(user_active_days, get_user_active_days(start_date, train_end_date, i), how='left',
                                        on=['user_id'])
                    #user_active_minutes = pd.merge(user_active_minutes, get_user_active_minutes(start_date, train_end_date, i), how='left',
                    #                    on=['user_id'])
                    user_other = pd.merge(user_other, get_user_other_feat(start_date, train_end_date, i), how='left',
                                        on=['user_id'])
                    actions = pd.merge(actions, get_action_feat(start_date, train_end_date, i), how='left',
                                        on=['user_id', 'sku_id'])
                    active_days = pd.merge(active_days, get_active_days(start_date, train_end_date, i), how='left',
                                        on=['user_id', 'sku_id'])
                    #active_minutes = pd.merge(active_minutes,get_active_minutes(start_date, train_end_date, i), how='left',
                    #                    on=['user_id', 'sku_id'])


        #actions = pd.merge(actions, user_all_action,   how='left', on='user_id')
        actions = pd.merge(actions, user,               how='left', on='user_id')
        actions = pd.merge(actions, user_conversion,    how='left', on='user_id')
        actions = pd.merge(actions, user_tm,            how='left', on='user_id')
        #actions = pd.merge(actions, user_type_tm,       how='left', on=['user_id'])
        actions = pd.merge(actions, user_last_buy_tm,   how='left', on='user_id')
        actions = pd.merge(actions, user_action,        how='left', on='user_id')
        actions = pd.merge(actions, user_active_days,   how='left', on='user_id')
        actions = pd.merge(actions, user_other,         how='left', on='user_id')
        #actions = pd.merge(actions, user_n_model,       how='left', on='user_id')
        #actions = pd.merge(actions, user_active_minutes,how='left', on='user_id')
        actions = pd.merge(actions, product,            how='left', on='sku_id')
        actions = pd.merge(actions, comment,            how='left', on='sku_id')
        actions = pd.merge(actions, product_conversion, how='left', on='sku_id')
        ...
        actions = pd.merge(actions, brand,              how='left', on='brand')
